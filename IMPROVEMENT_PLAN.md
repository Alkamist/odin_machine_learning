...
## Phase 4 — Re-layer the op set — DONE except 4.1b (2026-07-18)

- **4.1 CPU dedup:** cpu.odin 3749 → 3372 lines. 19 op families unified into `_foo_impl($T)` generics with `_typed_data`/`_load`/`_store` helpers (Bf16 loads to f32, accumulates f32, stores back — numerics unchanged; all suites + parity green before AND after). Left split with reasons: softmax fwd/bwd (structural differences), and linear/batched_matmul/attention (f32-SIMD vs bf16-scalar structure, plus a hard Odin constraint: **specialized generic procs cannot be passed as proc pointers to `parallelize` — compiles but segfaults**; direct calls only).
- **CUDA training coverage:** all 19 missing ops ported (F32): Sub, Div, Sqrt, Max, Mean, Transpose, Slice, Concat, Layernorm, Log_Softmax, Mean_Squared_Error, Smooth_L1, Relu, Sigmoid, Batched_Matmul (cuBLAS `GemmStridedBatchedEx`), Permute, Causal_Mask, Lerp_Assign, Accumulate_Mean (last two forward-only, matching CPU). ~15 would-be kernel files collapsed into 5 shared elementwise templates (kernels/elementwise/{unary,unary_back,binary,binary_back_a,binary_back_b}.cu) instantiated per-op via NVRTC -D options (possible because the pipeline cache keys by source+options). Notable: NVRTC lacks INFINITY — causal_mask uses `__int_as_float(0xff800000)`; Layernorm needed a CUDA `_alloc_scratch` case for mean/rstd. **The parity skip list is now empty — all 35 registry cases pass on GPU.** A relu+MSE MLP trains on CUDA.
- **Dropout / bias / embedding** (pure front-end composition, no new kernels): `dropout(input, rate)` builds a host-generated Bernoulli mask in a `scratch` tensor and multiplies (identity outside training; mask is a gradient sink; RNG seeding surface is Phase 6.2); `linear(input, weight, bias=…)` composes the broadcast `add`; `embedding(table, ids)` aliases `select`. mlp uses the bias param. `tests/dropout_check.odin` added.
- **Axis reductions:** new `Sum`/`Max_Reduce` trailing-reduce ops (CPU F32+Bf16, CUDA F32); `sum`/`mean`/`max_reduce` take `axis := -1` — non-trailing axes compose reshape→permute→trailing-reduce→reshape, output shape = input with the axis removed in original order. Max_Reduce backward recomputes argmax (first-index tie rule, identical on both backends). `min`/`max` now broadcast add-style on both backends (CUDA min migrated to the shared binary template; old min kernels deleted; `_assert_same_shape` removed as dead).
- **Convolution + pooling (NHWC):** ONE new backend op `Im2col` (+ col2im backward) makes `conv2d` a pure composition: im2col → existing `linear` (cuBLAS GEMM + optional trailing bias) → reshape. `conv1d` composes im2col directly (NOT via conv2d — a phantom h=1 axis would receive vertical padding). `Max_Pool2d`/`Avg_Pool2d` ops (no padding first cut; max backward recomputes argmax, first-index ties). CPU F32+Bf16, CUDA F32; cases + parity cover im2col (incl. padded), both pools; `tests/conv_check.odin` verifies a hand-computed conv2d.
- **4.1b (CUDA f32/bf16 kernel-pair collapse) DEFERRED to after Phase 5.2:** parity cases are F32-only today, so the CUDA bf16 kernels (the live LLM inference path) have zero test coverage — collapsing them now would be a mechanical refactor without a net. Order: add Bf16 parity cases (5.2), then collapse.
- Final state: 11 CPU tests green (also under ML_CPU_POISON), parity green with an empty skip list, all packages check clean.

Original plan below.

## Phase 4 (original) — Re-layer the op set (general-purpose core, LLM ops as extension)
...
1. **Sampling/generation utilities — DONE (2026-07-19).** New root package `sampling/`: `Sampler{temperature, top_k, top_p}`, `argmax`, `sample` (behavior-identical port of the llm_chat heap-select top-k → temperature softmax → top-p nucleus sampler; draws via `rand.float32()` so `context.random_generator` is the seeding surface pending 6.2), and `generate(eval, eval_data, prompt, logits, options, out_tokens) -> Generate_Stats` with `Generate_Options{sampler, max_new_tokens, prefill_chunk, stop_tokens, on_token, on_token_data}`. Prefill chunking lives in `generate` (`logits_out = nil` for non-final chunks — eval procs skip the logits copy on nil); a sampled stop token is counted in `decode_tokens` but neither appended nor passed to `on_token`. The package does not import `ml`; `ml.clear()` stays inside each backend eval. llm_chat shrank to wiring: `Chat_Model` swapped `is_stop` for `stop_tokens: []int` + `prefill_chunk`, streaming print became the `on_token` callback, the stats line reads `Generate_Stats`. Tests: `tests/sampling_check.odin` (argmax ties, greedy paths, seeded top-k membership and top-p dominance properties, scripted-eval generate test asserting chunk sizes, nil-logits pattern, stop handling, callback order, stats).
2. **RNG control — DONE (2026-07-19), no code changes needed.** The plan's premise was stale: every random call site in the library (`fill_normal`, the dropout mask, gemma/jepa randomize, `sampling.sample`) already goes through `core:math/rand`, which draws from `context.random_generator` — reproducibility was already expressible. Decision: bless that as THE seam and add no new API. **RNG contract:** all library randomness draws from `context.random_generator`; install a seeded generator (`state := rand.create(seed); context.random_generator = rand.default_random_generator(&state)`, or `rand.reset(seed)` on the default one) on the thread to make init, dropout masks, and sampling reproducible. Rejected: a generator field on `ml.Context` (duplicates the context system, and `sampling` deliberately doesn't import `ml`) and explicit generator params (viral, non-idiomatic). Verified in `base:runtime`: the default generator (`data = nil` in `default_random_generator_proc`) is a `@(thread_local)` ChaCha8 state — per-thread and race-free by construction, settling the Phase 7.2 question; CPU/CUDA both covered because dropout masks are host-generated. The deferred 5.7 determinism test now exists: `tests/determinism_check.odin` seeds a run (mlp make/randomize → 3 train steps with dropout on random batches → Adam updates) and asserts bit-identical parameters for equal seeds, differing parameters otherwise. Device-side Philox stays deferred until host-mask upload cost matters.
3. **Dataset/batching helpers — DONE (2026-07-19).** New root package `dataset/` (named `dataset`, not the plan's `data`, because `data` is a pervasive local-variable name that would shadow the import). `Batcher` owns a copy of its index set (`batcher_make` proc group over a sample count or an explicit `[]int`), `batcher_reset` reshuffles per epoch via `rand.shuffle` (so it obeys the 6.2 `context.random_generator` contract), `batcher_next` follows the core-library `(value, ok)` iterator convention and supports `drop_last`; `gather` is a generic strided row-gather into a caller staging buffer; `split(sample_count, validation_fraction)` returns independently-deletable shuffled train/validation index slices. mnist rewired: the old block-shuffle (`order` over batch indices — samples never remixed across epochs) is replaced by per-sample shuffling + gather, and `mnist_sample` is deleted. Verified: 39/39 tests green including `tests/dataset_check.odin` (sequential/partial batches, epoch-permutation properties under a seeded generator, index-copy ownership, strided gather, split disjointness/coverage), and a full 50-epoch mnist run converges to ~97.8% validation accuracy.
4. **Error handling at the library boundary — DONE (2026-07-19).** Typed errors only where a caller can react; everything else deliberately stays `bool` + `log.errorf` (the log carries the detail, the enum carries the category).
   - `gguf.Error`/`safetensors.Error` `{None, Not_Found, Read_Failed, Malformed, Unsupported}` on `load`; tokenizer (gemma/gpt2) `Error {None, Not_Found, Read_Failed, Malformed}` on `load`; `ml.Checkpoint_Error {None, Not_Found, Read_Failed, Malformed, Mismatch}` on `checkpoint_load` (per-tensor validation → `.Mismatch`; safetensors `.Malformed`/`.Unsupported` fold to `.Malformed`); `jepa.load` propagates `ml.Checkpoint_Error` (its dim validation → `.Mismatch`). `.Not_Found` enables resume-or-fresh-start and fetch-on-missing patterns and therefore MUST NOT log at error level (log.debugf only) — tested. Note: `safetensors.Error.Unsupported` is currently unreachable (the parser accepts unknown dtype strings; rejection happens later in `weights`, which is by design bool) — kept for gguf parity.
   - Stayed `bool` on purpose: `weights.write_tensor`, all network loaders (`gemma.load_gguf`/`load_safetensors`, `llama.load_safetensors`), `safetensors.save`, `checkpoint_save`, `jepa.save`, tokenizer `encode`, `mnist_load`. By the time tensors are wired into a model every failure is one category ("wrong file for this model"); a one-value error enum is ceremony.
   - **Backend failure contract (documented per the plan):** backend-internal CUDA/driver/NVRTC failures panic by design — this is research-harness pragmatics, they are not recoverable mid-graph. Programmer errors (shape/dtype misuse) assert with `#caller_location`. Only file/checkpoint I/O returns errors.
   - **Test-soundness discovery:** `core:testing.expect*` fail a test ONLY by logging at error level through `context.logger` — under `log.nil_logger()` every expectation is vacuous. The loader_check corruption suites had been unable to fail since Phase 5.6 (nil_logger installed for the whole helper); that masked a real category conflation (`!version_ok || version != 3` returned `.Unsupported` for version-field truncation). Fixed both: nil_logger is now scoped to the `load` call only (logger restored before expectations), the version check is split (short read → `.Malformed`, wrong version → `.Unsupported`), and the harness was proven live by sabotaging an expectation and observing the failure. Rule going forward: never call `testing.expect*` while a nil/quiet logger is installed.
   - Verified: all touched packages check clean; `tests` 39/39, `tests\golden` 4/4 (golden gemma tokenizer test now skips specifically on `.Not_Found` and fails on any other error).
...
7. **Config/toolchain robustness — DONE (2026-07-19).** The `CUDA_PATH` half was already in place (`_resolve_include_arg` probes the env var before the hardcoded fallbacks). Fast math is now per context: `cuda.context_create(fast_math := true)` stores the flag on the CUDA `Context`, `_compile_pipeline` emits `--use_fast_math` only when set, and the pipeline cache key gains a `no_fast_math` suffix so precise and fast pipelines never collide (the fast-math-on key stays byte-identical to before, preserving the zero-alloc fast path). `test_cuda_lifecycle` cycle 1 now runs with `fast_math=false` and executes a real `mul` forward, covering the precise-compile path and the distinct cache key. Verified: `tests` 39/39, parity 3/3 (single-threaded deterministic; one multi-threaded run hit the known pre-existing device-teardown race — Phase 7).
...
## Phase 7 — Threading contract — DONE (2026-07-19) except 7.5 and the optional clear_generation stamping

- **Root cause of the flaky GPU test segfault found and fixed:** device-lifetime globals were allocated with the ambient `context.allocator` — under the test runner, a per-test tracking allocator that dies with its test while the global lives on (Odin maps store their allocator, so a later rehash/free called a dead allocator). Fixed by allocating all device/pool-lifetime state with `runtime.default_allocator()`: the CUDA pipeline-cache map, its cloned keys, and `Pipeline` structs (`_device_init_locked`, `_compile_pipeline`, `_destroy_pipeline`, `device_destroy`), and the CPU worker pool (`_startup_thread_pool`/`_cleanup_thread_pool` set `context.allocator` before allocating, which also covers `thread.create`/`destroy` and makes the `@(fini)` teardown context safe). This simultaneously eliminated the documented parity-suite leak/bad-free tracker noise — the parity suite now runs with zero warnings, repeatedly green under the multi-threaded runner.
- **7.2:** `parallelize` now asserts against being called from inside a parallelize job (thread-local `_in_parallelize` set around chunk execution on workers AND the main thread's inline chunk — both re-entries would deadlock on the non-recursive pool mutex). RNG thread-safety and `cases.get()` were already settled (Phase 6.2 / 5.7); 1.7/1.8 landed in Phase 1.
- **7.3:** `ml.Context` gained `owner_thread_id`. `context_begin` claims it with an atomic compare-exchange (re-entry by the owning thread allowed, another thread asserts with the owner's id in the message); `context_end` releases when the ending context leaves the thread's stack; `_context_destroy` asserts the context is not active anywhere. Since `_current_ctx` is thread-local, the begin-time check is the complete enforcement — per-op stamping is unnecessary. The debug-define `clear_generation` transient stamping remains optional future work (catches stale-tensor-after-clear, an orthogonal single-threaded footgun).
- **7.4:** `tests/parallel_inference_check.odin` (CPU, with `set_thread_count(4)` so pool contention is real) and `tests/parity/parallel_gpu_check.odin` (GPU, `_cuda_ready`-gated): weights built under a main context, 4 threads each create their own context and run the identical inference forward, every output asserted bit-identical (u32 transmute) to the single-threaded reference; full create/destroy pairing per thread. Written by an Opus sub-agent; adversarial review caught one real defect — the workers were handed the spawning test's whole `context`, whose `temp_allocator` points at the spawner's thread-local temp arena (the CUDA dispatch path temp-allocates per op, so 4 workers would race on one bump arena). Fixed: workers keep their own default context and receive only the mutex-guarded tracking `context.allocator` via the payload.
- **7.1:** contract recorded in the new root `README.md` (Tiers 1–3, publish points for Tier 2 — `set_data` is synchronous, op-written persistent tensors publish at the writer's `ml.clear()`/`get_data` — pool serialization policy), together with the other written-down contracts (RNG, transient buffers, update gradient-zeroing, gradient sinks, error/panic split, device-lifetime allocator exemption). `tests/README.md` stale leak-noise note replaced.
- 7.5 (data-parallel training utility) stays future work as planned.
- Verified: root/cpu/cuda packages check clean; `tests` 40/40 (also under ML_CPU_POISON), parity 4/4 repeatedly green multi-threaded on the RTX 3090 Ti with zero tracker warnings.

Original plan below.

## Phase 7 (original) — Threading contract
