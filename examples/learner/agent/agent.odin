...
Score_Proc     :: proc(sensor: []f32) -> (reward: f32, dead: bool)
...
catch_up :: proc(a: ^Agent, time: f64) {
	for sync.atomic_load(&a.running) {
		if transmute(f64)sync.atomic_load(&a.published_processed_time) >= time - a.decision_period {
			return
		}
		thread.yield()
	}
