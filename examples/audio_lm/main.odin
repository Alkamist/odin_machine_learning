// Train a small Llama+QK-norm decoder on a flat EnCodec token stream
// produced by `tools/audio_lm_dump.py`. Vocab is 8192 (8 codebooks of
// 1024, interleaved with a per-codebook offset). No text conditioning.
// Periodically writes a sample of generated tokens that can be decoded
// back to audio with `tools/audio_lm_decode.py`.
//
//   odin run examples/audio_lm -o:speed -- train
//   odin run examples/audio_lm -o:speed -- generate out.bin [num_tokens]
//
// `train` saves the final weights to examples/data/audio_lm.ckpt.
// `generate` loads that checkpoint and writes a token .bin that
// audio_lm_decode.py can render to wav. num_tokens defaults to a few
// seconds; values larger than SEQ_LEN are produced via a sliding KV
// cache so RoPE positions stay in the trained range.
...
TRAIN_TOKENS    :: DATA_DIR + "/audio_train.bin"
VALID_TOKENS    :: DATA_DIR + "/audio_valid.bin"
SAMPLE_DIR      :: DATA_DIR + "/audio_samples"
CHECKPOINT_PATH :: DATA_DIR + "/audio_lm.ckpt"

// Frame rate constants from the EnCodec dumper, kept here so generate
// can convert "seconds" into a token count without round-tripping through
// the python tools.
ENCODEC_FRAME_RATE :: 75
TOKENS_PER_SECOND  :: ENCODEC_FRAME_RATE * NUM_CODEBOOKS
...
NUM_CODEBOOKS :: 8       // must match tools/audio_lm_dump.py
VOCAB_SIZE   :: 8192     // NUM_CODEBOOKS * 1024
SEQ_LEN      :: 1024     // ~1.7 s of audio at 600 tokens/s
ACCUM_STEPS  :: 4
TOTAL_STEPS  :: 8000
LOG_EVERY    :: 100
SAMPLE_EVERY :: 1000
...
SAMPLE_PRIME  :: 64                       // seed tokens copied from the corpus
...
run_train :: proc() {
	...
	fmt.println("Loading audio tokens ...")
...
run_generate :: proc(args: []string) {
	...
	num_tokens = (num_tokens / NUM_CODEBOOKS) * NUM_CODEBOOKS  // keep whole frames
	...
	prime_offset  = (prime_offset / NUM_CODEBOOKS) * NUM_CODEBOOKS
...
// This bounds RoPE positions to [0, SEQ_LEN), which is where the model
...
generate_rolling :: proc(model: llama.Llama, prime: []int, new_token_count: int, temperature: f32, top_k: int) -> []int {
	...
	for step_index in 0 ..< new_token_count {
		codebook_index := (builtin.len(prime) + step_index) % NUM_CODEBOOKS
		mask_to_codebook(last_logits, codebook_index)
		next_token := sample_token(last_logits, temperature, top_k)
		append(&all_tokens, next_token)

		// One slot reserved so we always have room to encode the new token
		// after the rebuild without overflowing the cache.
...
emit_sample :: proc(model: llama.Llama, valid_set: []int, out_path: string) {
	...
	prime_offset = (prime_offset / NUM_CODEBOOKS) * NUM_CODEBOOKS  // align to codebook-0 boundary
	...
	for step_index in 0 ..< SAMPLE_TOKENS {
		mask_to_codebook(last_logits, (prime_count + step_index) % NUM_CODEBOOKS)
...
// Force-zero probability outside the expected codebook range for this
// position. Disjoint codebook value ranges + occasional cross-range
// leakage from the model produces decoder glitches; this kills them.
mask_to_codebook :: proc(logits: []f32, codebook_index: int) {
	low  := codebook_index * 1024
	high := low + 1024
