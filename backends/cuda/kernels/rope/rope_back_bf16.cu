// Rope backward in bf16-data / F32-grad mode. Forward rotation doesn't need
// data inputs in backward (cos/sin recomputed), so dy/dx are pure f32 and
// this kernel matches rope_back_f32 byte-for-byte aside from the name.
extern "C" __global__
void rope_back_bf16(const float* __restrict__ dy,
