// Fused fp32 rmsnorm + rope + bf16 K-cache write. Same per-(token, head)
// layout as `rmsnorm_rope_f32.cu` but the rotated K row is converted to bf16
// and stored directly at the K-cache slot for `position_offset`. Mirrors
// ggml's `rope_neox` `set_rows_stride` branch.
//
//   slot_base = min(*pos_dev, capacity - token_count)
//   cache[(slot_base + token) * kv_size + head * head_size + ...] = bf16(rotated_pair)
//
// Cache is bf16 packed-pairs (2 bf16 per uint32). The kernel writes one uint32
// per pair-index per token-head.
#include <cuda_bf16.h>
...
extern "C" __global__
void rmsnorm_rope_cache_f32(const float*        __restrict__ x,
