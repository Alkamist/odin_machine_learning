...
test_bf16_round_to_nearest_even :: proc(t: ^testing.T) {
	...
	for c in cases {
		...
		got := _bf16_bits(x)
		testing.expectf(t, got == c.want_bf,
			"bf16 rte %s: in=0x%08x got=0x%04x want=0x%04x", c.name, c.in_bits, got, c.want_bf)
	}
}
...
test_bf16_nan_stays_nan :: proc(t: ^testing.T) {
	...
	for bits in nan_inputs {
		...
		testing.expectf(t, out != out,
			"bf16 nan: in=0x%08x produced non-NaN out=0x%08x", bits, transmute(u32)out)
		testing.expectf(t, !math.is_inf(out),
			"bf16 nan: in=0x%08x collapsed to infinity", bits)
	}
}
...
test_bf16_infinity_passthrough :: proc(t: ^testing.T) {
	...
	for c in cases {
		...
		bf  := ml.bf16_from_f32(x)
		out := ml.bf16_to_f32(bf)
		testing.expectf(t, u16(bf) == c.want_bf,
			"bf16 inf: in=0x%08x got bf=0x%04x want=0x%04x", c.in_bits, u16(bf), c.want_bf)
		testing.expectf(t, transmute(u32)out == c.want_f32,
			"bf16 inf: in=0x%08x round-tripped to 0x%08x want=0x%08x", c.in_bits, transmute(u32)out, c.want_f32)
		testing.expectf(t, math.is_inf(out),
			"bf16 inf: in=0x%08x is not infinity after round trip", c.in_bits)
	}
}
...
test_bf16_to_f32_exact :: proc(t: ^testing.T) {
	...
	for c in cases {
		...
		testing.expectf(t, transmute(u32)out == c.want,
			"bf16_to_f32: bf=0x%04x got=0x%08x want=0x%08x", c.bf, transmute(u32)out, c.want)
	}
}
...
test_bf16_exact_f32_round_trip :: proc(t: ^testing.T) {
	...
	for v in values {
		...
		testing.expectf(t, round == v,
			"bf16 round trip: value %v changed to %v", v, round)
	}
}
