...
main :: proc() {
	...
	// --- Phase 3: ml.linear forward + backward, CPU vs GPU ---
	{
		...
		for i in 0 ..< len(x_data) {
			x_data[i] = f32(i % 37) * 0.01 - 0.18
		}
		for i in 0 ..< len(w_data) {
			w_data[i] = f32((i * 11) % 53) * 0.005 - 0.13
		}
		...
		for i in 0 ..< len(cpu_y) {
			max_y_err  = math.max(max_y_err,  math.abs(cpu_y[i]  - gpu_y[i]))
		}
		for i in 0 ..< len(cpu_dx) {
			max_dx_err = math.max(max_dx_err, math.abs(cpu_dx[i] - gpu_dx[i]))
		}
		for i in 0 ..< len(cpu_dw) {
			max_dw_err = math.max(max_dw_err, math.abs(cpu_dw[i] - gpu_dw[i]))
		}
	...
	// --- Phase 5: ml.select ---
	{
		...
		for i in 0 ..< len(table_data) {
			table_data[i] = f32(i % 97) * 0.013 - 0.6
		}
		indices := [N]int{}
		for i in 0 ..< N {
			indices[i] = (i * 17 + 5) % VOCAB
		}
		...
		for i in 0 ..< len(cpu_dt) {
			max_dt_err = math.max(max_dt_err, math.abs(cpu_dt[i] - gpu_dt[i]))
		}
	...
	// --- Phase 8: ml.concat (3 inputs only on GPU) ---
	{
		...
		b_data := [ROWS * T_B]f32{}
		c_data := [ROWS * T_C]f32{}
		for i in 0 ..< len(a_data) {
			a_data[i] = f32(i % 31) * 0.02 - 0.3
		}
		for i in 0 ..< len(b_data) {
			b_data[i] = f32((i * 5) % 41) * 0.015 + 0.1
		}
		for i in 0 ..< len(c_data) {
			c_data[i] = f32((i * 11) % 53) * 0.01 - 0.2
		}
		...
		for i in 0 ..< len(cpu_da) {
			max_da_err = math.max(max_da_err, math.abs(cpu_da[i] - gpu_da[i]))
		}
		for i in 0 ..< len(cpu_db) {
			max_db_err = math.max(max_db_err, math.abs(cpu_db[i] - gpu_db[i]))
		}
		for i in 0 ..< len(cpu_dc) {
			max_dc_err = math.max(max_dc_err, math.abs(cpu_dc[i] - gpu_dc[i]))
		}
	...
	// --- Phase 10: ml.permute ---
	{
		...
		x_data := [D0 * D1 * D2]f32{}
		for i in 0 ..< len(x_data) {
			x_data[i] = f32((i * 13 + 5) % 67) * 0.02 - 0.4
		}
	...
	//   12b) broadcast (stride>1):   b is broadcast across the leading axis.
	{
		...
		// 12b: broadcast (stride > 1). a length = STRIDE * N_B; b length = N_B.
		{
			...
			b_data := [N_B]f32{}
			for i in 0 ..< N {
				a_data[i] = f32((i * 13) % 67) * 0.013 - 0.3
			}
			for i in 0 ..< N_B {
				b_data[i] = f32((i * 5)  % 29) * 0.04  + 0.05
			}
	...
	// --- Phase 13: ml.batched_matmul ---
	{
		...
		b_data := [B * K * N]f32{}
		for i in 0 ..< len(a_data) {
			a_data[i] = f32((i * 13) % 67) * 0.011 - 0.3
		}
		for i in 0 ..< len(b_data) {
			b_data[i] = f32((i * 7)  % 53) * 0.013 - 0.25
		}
	...
	// set_data hook (used by `scalar(1/sqrt(D))`).
	{
		...
		for i in 0 ..< len(x_data) {
			x_data[i] = f32((i * 13 + 5) % 71) * 0.013 - 0.45
		}
		...
		for i in 0 ..< len(cpu_dx) {
			max_dx = math.max(max_dx, math.abs(cpu_dx[i] - gpu_dx[i]))
		}
	...
	// --- Phase 15: ml.layernorm ---
	{
		...
		w_data := [SIZE]f32{}
		for i in 0 ..< len(x_data) {
			x_data[i] = f32((i * 11) % 73) * 0.027 - 0.9
		}
		for i in 0 ..< len(w_data) {
			w_data[i] = f32((i * 5)  % 19) * 0.05  + 0.5
		}
		...
		for i in 0 ..< len(cpu_dw) {
			max_dw = math.max(max_dw, math.abs(cpu_dw[i] - gpu_dw[i]))
		}
	...
	// --- Phase 16: ml.cross_entropy ---
	{
		...
		for i in 0 ..< len(x_data) {
			x_data[i] = f32((i * 11) % 89) * 0.04 - 1.6
		}
		targets := [COUNT]int{}
		for i in 0 ..< COUNT {
			targets[i] = (i * 13 + 7) % CLASS_SIZE
		}
		...
		max_loss, max_dx: f32
		for i in 0 ..< COUNT {
			max_loss = math.max(max_loss, math.abs(cpu_loss[i] - gpu_loss[i]))
		}
		for i in 0 ..< COUNT * CLASS_SIZE {
			max_dx  = math.max(max_dx,   math.abs(cpu_dx[i]   - gpu_dx[i]))
		}
	...
	// --- Phase 17: ml.mean ---
	{
		...
		x_data := [COUNT * SIZE]f32{}
		for i in 0 ..< len(x_data) {
			x_data[i] = f32((i * 13) % 71) * 0.025 - 0.7
		}
	...
	// confirm they agree at the fp32 reduction floor.
	{
		...
		for i in 0 ..< len(x_data) {
			x_data[i] = f32((i * 7) % 41) * 0.03 - 0.4
		}
		w0_init := [HIDDEN * IN_DIM]f32{}
		for i in 0 ..< len(w0_init) {
			w0_init[i] = f32((i * 11) % 53) * 0.02 - 0.3
		}
		w1_init := [OUT_DIM * HIDDEN]f32{}
		for i in 0 ..< len(w1_init) {
			w1_init[i] = f32((i * 17) % 47) * 0.025 - 0.4
		}
		targets := [BATCH]int{}
		for i in 0 ..< BATCH {
			targets[i] = (i * 3 + 1) % OUT_DIM
		}
		...
		TOL :: 1e-4
		max_w0, max_w1: f32
		for i in 0 ..< len(cpu_w0) {
			max_w0 = math.max(max_w0, math.abs(cpu_w0[i] - gpu_w0[i]))
		}
		for i in 0 ..< len(cpu_w1) {
			max_w1 = math.max(max_w1, math.abs(cpu_w1[i] - gpu_w1[i]))
		}
	...
	// --- Phase 21: broadcast sub / div ---
	{
		...
		for i in 0 ..< N_B {
			b_data[i] = f32((i * 5)  % 29) * 0.04  + 0.2 // keep b > 0 so div is well-defined
		}
	...
	// --- Phase 23: ml.slice ---
	{
		...
		x_data: [N]f32
		for i in 0 ..< N {
			x_data[i] = f32((i * 17) % 73) * 0.017 - 0.4
		}
	...
	// detach via tensor() so entropy receives genuine probabilities.
	{
		...
		raw: [COUNT * SIZE]f32
		for i in 0 ..< len(raw) {
			raw[i] = f32((i * 13) % 41) * 0.05 - 0.7
		}
		probs: [COUNT * SIZE]f32
		for r in 0 ..< COUNT {
			max_v := f32(-1e30)
			for c in 0 ..< SIZE {
				max_v = math.max(max_v, raw[r * SIZE + c])
			}
			sum: f32
			for c in 0 ..< SIZE {
	...
	// --- Phase 26: ml.mean_squared_error ---
	{
		...
		for i in 0 ..< len(pred_data) {
			pred_data[i]   = f32((i * 13) % 41) * 0.03 - 0.4
		}
		for i in 0 ..< len(target_data) {
			target_data[i] = f32((i * 7)  % 31) * 0.04 - 0.5
		}
		...
		for i in 0 ..< len(cpu_dpred) {
			max_dpred = math.max(max_dpred, math.abs(cpu_dpred[i] - gpu_dpred[i]))
		}
