...
make :: proc(layer_count, head_count, embedding_size, vocabulary_size: int, allocator := context.allocator) -> (transformer: Transformer) {
	...
	transformer.token_embeddings = ml.make(.F32, {vocabulary_size, embedding_size})

	for &layer in transformer.layers {
		layer.norm0_weight = ml.make(.F32, {embedding_size})
		layer.qkv_weight   = ml.make(.F32, {3 * embedding_size, embedding_size})
		layer.proj_weight  = ml.make(.F32, {embedding_size,     embedding_size})
		layer.norm1_weight = ml.make(.F32, {embedding_size})

		hidden_size := 4 * embedding_size
		layer.mlp_up_weight   = ml.make(.F32, {hidden_size,    embedding_size})
