	...
	for &layer, layer_idx in model.layers {
		...
		if !is_kv_shared_layer(cfg, layer_idx) {
			kv_ok := weights.write_tensor(&layer.k_proj_weight, source, fmt.tprintf("%v.self_attn.k_proj.weight", prefix), .Rope_Permute, cfg.num_key_value_heads, head_dim) &&
			         ...
			         _load_norm_permuted(source, layer.k_norm_weight, fmt.tprintf("%v.self_attn.k_norm.weight", prefix), head_dim, 1.0)
			if !kv_ok {
				return false
			}
		}
...
_load_norm_permuted :: proc(source: weights.Source, target: ml.Tensor, name: string, head_size: int, extra_scale: f32) -> bool {
	values, info, ok := weights.read_floats(source, name)
	if !ok {
		return false
	}
	if !slice.equal(info.shape, []int{head_size}) {
		log.errorf("%q source shape %v expected [%v]", name, info.shape, head_size)
		return false
	}
	if target.rank != 1 || target.shape[0] != head_size {
		log.errorf("%q expected 1-D [%v], got rank=%v shape[0]=%v", name, head_size, target.rank, target.shape[0])
		return false
	}
...
_load_per_layer_embedding_host :: proc(loader: safetensors.Loader, model: Gemma) -> bool {
	...
	if !slice.equal(info.shape, expected_shape) {
		log.errorf("%q shape %v != expected %v", name, info.shape, expected_shape)
		return false
	}
	...
	case .Bf16:
		...
		case:
			log.errorf("%q unsupported source dtype %q", name, info.dtype)
			return false
		}
	case .F32:
		switch info.dtype {
